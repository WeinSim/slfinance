...
impl Expression {
    ...
    pub fn parse(input: &str) -> Result<Self, String> {
        let mut iter = CharIter::new(input.chars().filter(|c| !c.is_whitespace()).collect());
        if !iter.has_next() {
    ...
    fn parse_expression(iter: &mut CharIter) -> Result<Self, String> {
        let mut terms: Vec<Term> = Vec::new();
        while let Some(c) = iter.next() {
            match c {
                '+' | '-' => {
                    terms.push(Term {
                        expression: Self::parse_term(iter)?,
                        sign: if c == '+' {
                            Sign::Positive
                        } else {
                            Sign::Negative
    ...
    fn parse_term(iter: &mut CharIter) -> Result<Self, String> {
        match iter.peek() {
            Some('(') => {
                iter.next();
                Ok(Self::parse_expression(iter)?)
            }
            Some(_) => Ok(Self::Value(Self::parse_money(iter)?)),
    ...
    fn parse_money(iter: &mut CharIter) -> Result<Money, String> {
        ...
        while let Some(c) = iter.peek() {
            state = match state {
                State::Idle => State::Euros(Self::parse_int(iter)?),
