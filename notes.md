...
- slf edit
    ...
    - Change / remove: amount, date, category, description (basically
      everything).
    - Either via subcommands (slf edit date) or using the existing arguments
    (slf edit -D). Probably the latter
- slf list
...
### Problems to fix
- Sort MoneyChanges by category, then by date
- SerialTracker is 99% identical to Tracker. I think the only difference is that
YearMonth is flattened.
